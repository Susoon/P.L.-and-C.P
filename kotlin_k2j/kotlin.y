...
class_stt:	ABST CLASS ID OPEN class_param CLOSE c_inheritance M_OPEN class_decl M_CLOSE
	  	{
			...
			child = make_token_node("(", NOT_SPACE);	
			Add_First(child, NOT_NEED);
			Add_First($5, NOT_NEED);
			child = make_token_node(")", NEED_SPACE);
			Add_First(child, NOT_NEED);
			Add_First($7, NOT_NEED);
			child = make_token_node("{\n", NOT_SPACE);
			Add_First(child, NOT_NEED);
			Add_First($9, NOT_NEED);
			child = make_token_node("}", NOT_SPACE);
			Add_First(child, NOT_NEED);
	...
	|	CLASS ID OPEN class_param CLOSE c_inheritance M_OPEN class_decl M_CLOSE
	  	{
			...
			child = make_token_node("(", NOT_SPACE);
			Add_First(child, NOT_NEED);
			Add_First($4, NOT_NEED);
			child = make_token_node(")", NEED_SPACE);
			Add_First(child, NOT_NEED);
			Add_First($6, NOT_NEED);
			child = make_token_node("{\n", NOT_SPACE);
			Add_First(child, NOT_NEED);
			Add_First($8, NOT_NEED);
			child = make_token_node("}", NOT_SPACE);
			Add_First(child, NOT_NEED);
...
//			Add_First($3, NEED);
//			Add_First($1, NOT_NEED);
//			tmp_node = for_condition -> child;
//			Add_First(tmp_node, NOT_NEED);
//			Add_First($1, NOT_NEED);
//			Add_First(tmp_node -> next, NEED);
//			Add_First($4, NOT_NEED);
...
is_condition :	ID IS type	
	...
	;
range	:	DOUBLEDOT cal_sent step_count 
      		{
			parent = make_nt_node();
			parent -> type = Bool;
			if(for_condition == NULL)
			...
			tmp_node = $3;
			child = make_token_node("+=", NEED_SPACE);
			for_condition -> child = child;
			for_condition -> child -> next = $3;
...
void init_for_condition()
{
	if(for_condition != NULL)
		return;
	for_condition = (NODE*)malloc(sizeof(NODE));
	for_condition -> name = NULL;
	for_condition -> classtype = NULL;
	for_condition -> child = NULL;
	for_condition -> next = NULL;
	for_condition -> prev = NULL;
	for_condition -> parent = NULL;
	for_condition -> type = -10000;
}
